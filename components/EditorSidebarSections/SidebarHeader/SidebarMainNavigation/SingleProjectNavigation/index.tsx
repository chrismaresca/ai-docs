...
import { useAiInstanceContext, useConversationContext } from "@/context";

import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";

const SingleProjectNavigation: React.FC = () => {
  const { updateAiInstance } = useAiInstanceContext();
  const { showConversation, toggleShowConversation } = useConversationContext();
  ...
  return (
    <>
      <div className="w-full  h-9 rounded-md inline-flex justify-between align-middle items-center shadow-sm shadow-slate-300 border border-border/60">
        <div className="inline-flex justify-center items-center text-xs group hover:cursor-pointer" onClick={() => updateAiInstance(null, null)}>
          <ChevronLeft className="h-2.5 w-2.5 ml-2 mr-1 group-hover:scale-[1.1]" />
          <div className="group-hover:underline">All Edits</div>
        </div>
