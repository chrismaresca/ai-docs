...
interface InstancesListViewProps {
  inactiveIterations: TextIteration[];
}

const TextIterationListView: React.FC<InstancesListViewProps> = ({ inactiveIterations }) => {
  ...
  return (
    <div className="w-full">
      {inactiveIterations.length == 0 ? (
        <>0 </>
      ) : inactiveIterations.length == 1 ? (
        <>1</>
      ) : (
        <>
          ...
          <div className={`my-1 space-y-1 transition-max-height duration-500 ${isIterationSectionExpanded ? "max-h-24 overflow-y-auto" : "max-h-0 overflow-y-hidden"}`}>
            {inactiveIterations.map((iteration) => (
