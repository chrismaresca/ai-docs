...
function AlignmentCombobox() {
  ...
  const handleSelect = useCallback(
    (currentValue: string) => {
      setValue(currentValue === "" || currentValue === null ? "left" : currentValue);
      setOpen(false);
          ...
          <CommandList>
            <CommandGroup>
              {alignments.map((align) => (
                <CommandItem key={align.value} value={align.value} onSelect={() => handleSelect(align.value)} className={`${value === align.value ? "bg-gray-100" : ""}`}>
